...
for zone = 1:2;
    ...
    elseif zone == 2;
        DistEnd = DistIni + 20; %that's 25 to 45
        DistEndStroke = DistIniStroke + 25; %That's 25 to 50
    end;
...
%                         end;
    ...
    if BOAll(lap,1)+RaceStart > liEnd;
        ...
        if zone == 1;
            if isnan(liDistLap(indexIniStroke,3)) == 1 | isnan(liDistLap(indexEnd,3)) == 1;
                isInterpolatedSplits(lap,pos(zone)) = 1;
            end;
            SectionSplitTime(lap,pos(zone)) = TimeEnd-TimeIniStroke;
            SectionCumTime(lap,pos(zone)) = TimeEnd;
            SectionSplitTimebis(lap,pos(zone)) = TimeEnd-TimeIniStroke;
            SectionCumTimebis(lap,pos(zone)) = TimeEnd;
        else;
            if isnan(liDistLap(indexIniStroke,3)) == 1 | isnan(liDistLap(indexEndStroke,3)) == 1;
                isInterpolatedSplits(lap,pos(zone)) = 1;
            end;
            SectionSplitTime(lap,pos(zone)) = TimeEndStroke-TimeIniStroke;
            SectionCumTime(lap,pos(zone)) = TimeEndStroke;
            SectionSplitTimebis(lap,pos(zone)) = TimeEndStroke-TimeIniStroke;
            SectionCumTimebis(lap,pos(zone)) = TimeEndStroke;
        end;
...
%                                         ########################
                    ...
                    StrDuration = (StTime2Stroke - StTime1Stroke);
                    SectionSR(lap,pos(zone)) = roundn(((length(IDstrokeStroke)-1).*60)./StrDuration, -1);
                    SectionSRbis(lap,pos(zone)) = roundn(((length(IDstrokeStroke)-1).*60)./StrDuration, -1);
...
%                     VelEC = SectionVel(lap,pos(zone));
                    ...
                    if isempty(IDstrokeOther) == 0;
                        StTime1Stroke = (IDstrokeOther(1)-listart)./framerate;
                        StTime2Stroke = (IDstrokeOther(end)-listart)./framerate;
                    else;
                        StTime1Stroke = (IDstrokeStroke(1)-listart)./framerate; %Take all the stroke in the segment (0 to 25 or 25 to 50)
                        StTime2Stroke = (IDstrokeStroke(end)-listart)./framerate;
                    end;
                    ...
                    if lap >= 2;
                        if zone == 1;
                            ...
                            SegTime1 = (liIniStroke-listart)/framerate;
                        end;
                    else;
                        if zone == 1;
                            SegTime1 = (liIniRef-listart)/framerate; %%take the 15m mark to extrapolate
                        else;
                            SegTime1 = (liIniStroke-listart)/framerate;
                        end;
                    end;
                    ...
                    PosTime1Stroke = (StTime1Stroke - SegTime1) .* VelEC;
                    PosTime2 = (StTime2 - SegTime2) .* VelEC;
                    PosTime2Stroke = (StTime2Stroke - SegTime2) .* VelEC;

                    if lap >= 2;
                        if zone == 1;
                            StDist1 = DistIni + PosTime1; %take the 40m mark (not DistIniRef) to extrapolate the position
                            StDist2 = DistEndRef + PosTime2; 
                            StDist1Stroke = DistIni + PosTime1Stroke; %take the 40m mark (not DistIniRef) to extrapolate the position
                            StDist2Stroke = DistEndRef + PosTime2;
                        else;
                            StDist1 = DistIniRef + PosTime1;
                            StDist1Stroke = DistIniStroke + PosTime1Stroke;
                            StDist2 = DistEndRef + PosTime2;
                            StDist2Stroke = DistEndRef + PosTime2Stroke;
                        end;
                    else;
                        if zone == 1;
                            StDist1 = DistIni + PosTime1; %from 15
                            StDist1Stroke = DistIni + PosTime1Stroke;
                            StDist2 = DistEndRef + PosTime2;
                            StDist2Stroke = DistEndRef + PosTime2Stroke;
                        else;
                            StDist1 = DistIniRef + PosTime1;
                    ...
                    if isempty(IDstrokeOther) == 0;
                        if IDstrokeOther == 0;
                            ...
                            SectionSD(lap,pos(zone)) = (StDist2 - StDist1) ./ (length(IDstrokeOther)-1);
                            SectionSD(lap,pos(zone)) = roundn(SectionSD(lap,pos(zone)), -2);
                            SectionSDbis(lap,pos(zone)) = (StDist2 - StDist1) ./ (length(IDstrokeOther)-1);
                            SectionSDbis(lap,pos(zone)) = roundn(SectionSD(lap,pos(zone)), -2);
                        end;
                        SectionNb(lap,pos(zone)) = length(IDstrokeOther);
                        SectionNbbis(lap,pos(zone)) = length(IDstrokeOther);
                    else;
                        SectionSD(lap,pos(zone)) = (StDist2Stroke - StDist1Stroke) ./ (length(IDstrokeStroke)-1); %to check if he needs to be rounded before or after the multiplication
                        SectionSD(lap,pos(zone)) = roundn(SectionSD(lap,pos(zone)),-2);
                        SectionSDbis(lap,pos(zone)) = (StDist2 - StDist1Stroke) ./ (length(IDstrokeStroke)-1);
                        SectionSDbis(lap,pos(zone)) = roundn(SectionSDbis(lap,pos(zone)),-2);
                        SectionNb(lap,pos(zone)) = length(IDstrokeStroke);
                        SectionNbbis(lap,pos(zone)) = length(IDstrokeStroke);
                    end;
                end;
                ...
                if isempty(lilap) == 1;
                    ...
                    if length(liStrokeSecRef) >= 2;
                        ...
                        if length(liStrokeSecStroke) == 2;
                            StrDuration = abs((StTime3Stroke - StTime1Stroke));
                            SectionSR(lap,pos(zone)) = ((length(IDstrokeStroke)).*60)./StrDuration;
                            SectionSRbis(lap,pos(zone)) = ((length(IDstrokeStroke)).*60)./StrDuration;
                        else;
                            StrDuration = (StTime2Stroke - StTime1Stroke);
                            SectionSR(lap,pos(zone)) = ((length(IDstrokeStroke)-1).*60)./StrDuration;
                            SectionSRbis(lap,pos(zone)) = ((length(IDstrokeStroke)-1).*60)./StrDuration;
                        end;
...
%                         VelEC = SectionVel(lap,pos(zone));
                        ...
                        if isempty(IDstrokeOther) == 0;
                            if IDstrokeOther == 0;
                                ...
                                SectionSDbis(lap,pos(zone)) = (StDist2 - StDist1) ./ (length(IDstrokeOther)-1);
                                SectionSDbis(lap,pos(zone)) = roundn(SectionSD(lap,pos(zone)) .* 2, -2);
                            end;
                            SectionNb(lap,pos(zone)) = length(IDstrokeOther);
                            SectionNbbis(lap,pos(zone)) = length(IDstrokeOther);
                        else;
